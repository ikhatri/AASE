...
def setup_traffic_DBN(filepath: Path):
    ...
    light_model = setup_model(filepath.joinpath("single_light_model.csv"), epsilon=0.02)
    ...
    dbn.add_edges_from(
        [
            (("Traffic Light", 0), ("Traffic Light", 1)),
...
def setup_backbone_DBN(filepath: Path):
    system_model = setup_model(filepath.joinpath("system_model.csv"), epsilon=0.01)
    our_model = setup_model(filepath.joinpath("our_light_model.csv"), epsilon=0.005)
    cross_model = setup_model(filepath.joinpath("cross_light_model.csv"), epsilon=0.005)
    vision_model = setup_model(filepath.joinpath("vision_evidence.csv"), epsilon=0.08)
    dbn = DBN()
    our_light = "Our Light"
