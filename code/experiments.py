        ...
        for log_id in relevant_cars:
            if relevant_cars[log_id].get("skip", False) is False:
                ...
                for i in range(len(evidence_dict)):
                    if i in adj_obj_ids or i in cross_obj_ids:
                        ...
                        for t in discr_evidence_dict:
                            key, value, timestep = convert_pgmpy_pom(
                                t, discr_evidence_dict[t]
                            )
                            pom_evidence_dicts[timestep][key] = value
                pom_evidence_dicts.pop(0)

                ft, yolo = parse_yolo(
                    ARGOVERSE_TRACKING.joinpath(folder + "/" + log_id + "/rfc.txt")
                )
                ...
                for i, evidence in enumerate(pom_evidence_dicts):
                    ...
                    next_belief, out = predict_DBN(
                        dbn, names, evidence, i + 1, iterations=7
                    )
                    pom_out = pom_out + out
