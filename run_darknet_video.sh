...
if [ -z "$3" ]
then
    cd $DARKNET && ./darknet detector demo data/tl.data cfg/yolov3-tl.cfg backup/yolov3-tl_final.weights $VIDEO/rfc.mp4 -out_filename yolo_output.avi > rfc.txt && mv yolo_output.avi rfc.txt $VIDEO
else
